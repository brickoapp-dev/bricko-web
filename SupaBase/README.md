...
| `…_request_images.sql` | **Aplicada parcialmente 2026-08-13** (la parte de policies/bucket; falta un paso manual, ver abajo). No agrega columna — `requests.fotos text[]` ya existía. Encontró DOS intentos previos incompletos hechos a mano: bucket `request-photos` (privado, folder=uploader, un profesional no podía ver fotos ajenas) y bucket `solicitudes` (público, cualquiera veía cualquier foto). Se dejó `solicitudes` como bucket único, ahora privado, con policies calcadas de `requests_select` (dueño o profesional mientras está abierta). **Pendiente:** borrar el bucket `request-photos` (vacío, sin uso) — el SQL de borrado está en el archivo pero no se ejecutó porque toca `storage.objects`/`storage.buckets` y quedó para confirmación explícita. |
